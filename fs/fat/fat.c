	...
	do {
		...
		while(actsize < filesize) {
			...
			if((newclust -1)!=endclust)
				goto getit;
			/***** YWDRIVER_MODI 2010-09-28 D26LF Modi:
			    Description:Add support for valid cluster number for FAT32
...
read_bootsectandvi(boot_sector *bs, volume_info *volinfo, int *fatsize)
{
	...
	/*****     2012-05-09     *****/
	//YWDRIVER_MODI modify by lf for ±àÒë¸æ¾¯ start
	//´íÎó£ºÊý×éÏÂ±ê´óÓÚÊý×é±ß½ç
	#if 1
	*(vistart->fs_type + 8) = '\0';
	#else
	vistart->fs_type[8] = '\0';
