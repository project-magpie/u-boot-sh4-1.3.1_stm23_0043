...
#define YWVFD_Warn(char* str)
#define YWVFD_U_MENU(void)
#define YWVFD_BOOT_P(pro_rate)
#define YWVFD_KERNEL_P(pro_rate)
#define YWVFD_Print(char* str)
#define YWVFD_GetScanKey(int *key);
#endif
...
static int	set_system_spark(char *p)
{
	YWVFD_Print("SPAr");
	if(p && !strcmp(p, "spark"))
	{
	...
	else
	{
		...
		setenv("bootargs", "console=ttyAS1,115200 rw ramdisk_size=6144 init=/linuxrc root=/dev/ram0 nwhwconf=device:eth0,hwaddr:00:80:E1:12:40:69 ip=192.168.40.69:192.168.3.119:192.168.3.1:255.255.0.0:lh:eth0:off stmmaceth=msglvl:0,phyaddr:1,watchdog:5000 bigphysarea=4000");
		setenv("bootargs_spark", "console=ttyAS1,115200 rw ramdisk_size=6144 init=/linuxrc root=/dev/ram0 nwhwconf=device:eth0,hwaddr:00:80:E1:12:40:69 ip=192.168.40.69:192.168.3.119:192.168.3.1:255.255.0.0:lh:eth0:off stmmaceth=msglvl:0,phyaddr:1,watchdog:5000 bigphysarea=4000");
	}
	...
	setenv("boot_system", "spark");
	setenv("bootcmd", "bootm " CFG_YW_KERNEL_BASE);
...
static int	set_system_enigma2(char *p)
{
	YWVFD_Print("ENIG");
	...
	else
	{
		...
		setenv("bootargs", "console=ttyAS1,115200 root=/dev/mtdblock3 rootfstype=jffs2 rw init=/bin/devinit coprocessor_mem=4m@0x40000000,4m@0x40400000 printk=1 nwhwconf=device:eth0,hwaddr:00:80:E1:12:40:61 rw ip=172.100.100.249:172.100.100.174:172.100.100.174:255.255.0.0:LINUX7109:eth0:off bigphysarea=6000 stmmaceth=msglvl:0,phyaddr:2,watchdog:5000");
		setenv("bootargs_enigma2", "console=ttyAS1,115200 root=/dev/mtdblock6 rootfstype=jffs2 rw init=/bin/devinit coprocessor_mem=4m@0x40000000,4m@0x40400000 printk=1 nwhwconf=device:eth0,hwaddr:00:80:E1:12:40:61 rw ip=172.100.100.249:172.100.100.174:172.100.100.174:255.255.0.0:LINUX7109:eth0:off bigphysarea=6000 stmmaceth=msglvl:0,phyaddr:2,watchdog:5000");
	}
...
static int rs232_upgrade_uboot(void)
{
	...
	rcode = Call_command("protect off $monitor_sec;erase $monitor_sec;cp.b $load_addr $monitor_base $monitor_len;protect on $monitor_sec", 0);
	if(rcode != 0)
	{
		goto error;
	}
...
static int factory_update_flash(void)
{
	int rcode;

	rcode = Call_command ( "fatload usb 0 $load_addr st7111_flash_8m.bin", 0 );
	if(rcode != 0)
	{
	    rcode = Call_command ("tftp $load_addr st7111_flash_8m.bin", 0 );
        if (rcode != 0)
        {
		    goto error;
        }
	}
	rcode = Call_command("protect off all;erase all;cp.b $load_addr $monitor_base $filesize", 0);
	if(rcode != 0)
	{
		goto error;
	}
...
static int force_factory_mode(void)
{
	...
	rcode = Call_command ( "fatload usb 0 $load_addr $kernel_name", 0 );
	if(rcode != 0)
	{
	    rcode = Call_command ("tftp $load_addr $tftp_kernel_name", 0 );
        if (rcode != 0)
        {
		    goto error;
        }
	...
	rcode = Call_command ( "fatload usb 0 $load_addr $userfs_name", 0 );
	if(rcode != 0)
	{
	    rcode = Call_command ("tftp $load_addr $tftp_userfs_name", 0 );
        if (rcode != 0)
        {
		    goto error;
        }
	...
	if(!strcmp(p, "spark"))
	{
		rcode = Call_command ("nand write.yaffs2 $load_addr $userfs_base $filesize", 0 );
	}
	else
	{
		rcode = Call_command ("nand write.jffs2 $load_addr $userfs_base $filesize", 0 );
	}
	if(rcode != 0)
	{
		goto error;
...
static int	force_system_mode(void)
{
	...
	while(1)
	{
		...
		{
    		case	RAW_KEY_UP:
    			if(set_system_spark(p))
    			{
    				goto error;
    			}
    			break;
    		case	RAW_KEY_DOWN:
    			if(set_system_enigma2(p))
    			{
    				goto error;
    			}
    			break;
...
#ifdef YW_CONFIG_VFD
	#define BOOL int
	extern BOOL YWVFD_STANDBY;
	if(YWVFD_STANDBY)	udelay_num_max = 30;
#endif
//YWDRIVER_MODI d48zm 2009/12/18 for bootdelay end
	...
	if (bootdelay >= 0) {
        ...
        {
			if((RAW_KEY_OK == key) || (RAW_KEY_POWER == key))
			{
				...
				YWVFD_Print("SUCC");
				bootdelay = 3; // delay 3 seconds
				goto continue_boot;
			}
			else if (RAW_KEY_MENU == key || (RAW_KEY_LEFT == key))
			{
				abort  = 1; /* don't auto boot	*/
                #ifdef CONFIG_MENUKEY
...
# endif
            ...
            if(YWVFD_HasKeyPress(&key))
            {
                if (RAW_KEY_MENU == key || (RAW_KEY_LEFT == key))
    			{
                    abort  = 1;	/* don't auto boot	*/
                    bootdelay = 0;	/* no more delay	*/
                    #ifdef CONFIG_MENUKEY
...
//#define __TEST_FLASH__
#ifdef __TEST_FLASH__
static void rewrite(int start, int stop, int len)
{
    char cmd[256];

    sprintf(cmd, "cp.b %x $load_addr %x", start, len);
    //printf("%s\n", cmd);
    Call_command(cmd, 0);

    sprintf(cmd, "protect off %x %x", start, stop);
    //printf("%s\n", cmd);
    Call_command(cmd, 0);

    sprintf(cmd, "erase %x %x", start, stop);
    //printf("%s\n", cmd);
    Call_command(cmd, 0);

    sprintf(cmd, "cp.b $load_addr %x %x", start, len);
    //printf("%s\n", cmd);
    Call_command(cmd, 0);

    sprintf(cmd, "protect on %x %x", start, stop);
    //printf("%s\n", cmd);
    Call_command(cmd, 0);
}
...
void test_flash(void)
{
    ...
    for (i = 0; i < 127; i++)
    {
        sec[i][0] = 0xA0000000 + 0x10000 * i;
        sec[i][1] = 0xA0000000 + 0x10000 * i + 0xffff;
        len[i] = 0x10000;
    }
    for (i = 0; i < 8; i++)
    {
        sec[127 + i][0] = 0xA07F0000 + 0x2000 * i;
        sec[127 + i][1] = 0xA07F0000 + 0x2000 * i + 0x1fff;
        len[127 + i] = 0x2000;
    }
    for (i = 0; i < 135; i++)
    {
        //printf("sec[%d] = 0x%x 0x%x,len = 0x%x \n", i, sec[i][0], sec[i][1], len[i]);

    	char	str[5];
    	sprintf(str,"R%03d", i);
    	YWVFD_Print(str);

		rewrite(sec[i][0], sec[i][1], len[i]);
    }
    Call_command("reset", 0);
}

void test_flash_whole(void)
{
    int sec[1][2];
    int len[1];
	int Key;
	...
	while (1)
	{
        //printf("sec[%d] = 0x%x 0x%x,len = 0x%x \n", i, sec[i][0], sec[i][1], len[i]);
    	char	str[5];
    	sprintf(str, "%04x", iCount);

    	YWVFD_Print(str);

		rewrite(sec[0][0], sec[0][1], len[0]);

		printf("times %08d is ok\n", iCount++);
...
# ifdef CONFIG_YW_BURNER
	Call_command("flinfo", 0);
	Call_command("nand info", 0);

	force_factory_mode();
#endif

//YWDRIVER_MODI d48zm 2010/4/2 add
{
		ssize_t ret = 0;
...
#define MAX_CMDBUF_SIZE		1024 //YWDRIVER_MODI d02sh ex-leng the env for kernel start parms
