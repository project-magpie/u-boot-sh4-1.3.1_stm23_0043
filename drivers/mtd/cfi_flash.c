...
//YWDRIVER_MODI D02SH 2009/07/20 modify begin
//Description:add by sqy long long ago
extern unsigned int filetype ;
//YWDRIVER_MODI D02SH 2009/07/20 modify end
/*
...
#include "vfd.h"
static void flash_show_progress(char c, int percent)
{
	char	str[5];

	if(percent == 100) percent = 99;

	sprintf(str,"S%c%02d", c, percent);
	YWVFD_Print(str);
}
	...
	for (sect = s_first; sect <= s_last; sect++) {
		if (info->protect[sect] == 0) { /* not protected */
			...
			if (flash_full_status_check
                ...
                if (s_first == s_last)
                {
				    flash_show_progress('1', (sect - s_first )*100/(1));
                }
                else
                {
				    flash_show_progress('1', (sect - s_first )*100/(s_last - s_first));
                }

			}
		}
	}
	if (filetype == 0){ //YWDRIVER_MODI d02sh copy from ago
	puts (" done\n");
	}
	return rcode;
}
...
// YWDRIVER_MODI cc add for progress begin
		if ((wp % dot_stride) == 0x0ul)
		{
			putc ('.');
			flash_show_progress('2', (save_size - cnt)*100/save_size);
		}
// YWDRIVER_MODI cc add for progress end
	}
