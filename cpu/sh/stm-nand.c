...
/***** YWDRIVER_MODI 2010-09-30 D26LF Modi:
    Description:为了与Linux 原有文件系统兼容
*/
#if 1
...
/**
 * stm_flex_verify_buf - [DEFAULT] Verify chip data against buffer
 * @mtd:	MTD device structure
 ...
 */
extern int stm_flex_verify_buf(struct mtd_info *mtd, const u_char *buf, const int len)
{
	int i;

	for (i=0; i<len; i++)
		if (buf[i] != stm_flex_read_byte(mtd))
			return -EFAULT;
